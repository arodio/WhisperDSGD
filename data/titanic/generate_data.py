...
def main():
    ...
    dataset =\
        ConcatDataset([
            TitanicDataset(os.path.join(RAW_DATA_PATH, "train.csv")),
            TitanicDataset(os.path.join(RAW_DATA_PATH, "test.csv"))
        ])
    ...
    # Split clients into train and test tasks
    ...
    # Distinguish between training and test indices
    n_train = len(dataset.datasets[0])
    ...
    for mode, clients_indices in [('train', train_clients_indices), ('test', test_clients_indices)]:
        for client_id, indices in enumerate(clients_indices):
            ...
            train_indices = indices[indices < n_train]
            test_indices = indices[indices >= n_train]

            if (len(train_indices) == 0) or (len(test_indices) == 0):
                continue

            client_path = os.path.join(PATH, mode, f"task_{client_id}")
